  ...
  if (order.status !== 'cancelled' && order.status !== 'completed') {
    ...
    if (order.status === 'confirmed' || order.status === 'processing' || order.status === 'ready-delivery') {
      if (order.deliveryStatus !== 'delivered') {
        buttons.push(`<button onclick="window.generateDeliveryAction('${order.id}')" class="bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-semibold px-3 py-2 rounded-xl transition-all shadow-md shadow-emerald-500/10">Generate Delivery</button>`);
      }
...
window.generateDeliveryAction = function(orderId) {
  ...
  // Partial shipment prompt
  const itemsSelectHtml = order.items.map((item, idx) => `
    <div class="border-b border-slate-100 py-2 flex items-center justify-between text-xs font-semibold text-slate-700">
      <span>${escapeHtml(item.name)} (Ordered: ${item.quantity})</span>
      <input type="number" min="0" max="${item.quantity}" value="${item.quantity}" class="w-20 px-2 py-1 border border-slate-200 rounded-lg text-center focus:outline-none delivery-qty-input" data-index="${idx}">
    </div>
  `).join('');

  const modalOverlay = document.createElement('div');
  modalOverlay.className = 'fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-50 flex items-center justify-center p-4';
  modalOverlay.innerHTML = `
    <div class="bg-white rounded-3xl p-6 border border-slate-200 premium-shadow max-w-md w-full space-y-4">
      <h3 class="text-sm font-extrabold text-slate-900">Generate Dispatch Shipment Delivery</h3>
      <p class="text-xs text-slate-500 font-medium">Verify quantities below to generate shipment dispatch logs.</p>
      
      <div class="space-y-3 max-h-60 overflow-y-auto pr-1">
        ${itemsSelectHtml}
      </div>

      <div class="grid grid-cols-2 gap-3 pt-2 text-xs font-semibold">
        <div>
          <label class="block mb-1 text-[10px] text-slate-400 uppercase">Carrier / Person</label>
          <input type="text" id="delivery-carrier" class="w-full px-3 py-2 border border-slate-200 bg-slate-50 rounded-xl focus:outline-none" value="DHL Express">
        </div>
        <div>
          <label class="block mb-1 text-[10px] text-slate-400 uppercase">Tracking Number</label>
          <input type="text" id="delivery-tracking" class="w-full px-3 py-2 border border-slate-200 bg-slate-50 rounded-xl focus:outline-none" value="TRK${Math.floor(1000000 + Math.random()*9000000)}">
        </div>
      </div>

      <div class="flex justify-end gap-3 pt-3">
        <button type="button" id="delivery-cancel-btn" class="border border-slate-200 bg-white hover:bg-slate-50 text-slate-700 text-xs font-semibold px-4 py-2 rounded-xl transition-all">Cancel</button>
        <button type="button" id="delivery-submit-btn" class="bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-semibold px-4 py-2 rounded-xl transition-all shadow-md shadow-emerald-500/10">Dispatch</button>
      </div>
    </div>
  `;
  ...
  document.getElementById('delivery-cancel-btn').addEventListener('click', () => modalOverlay.remove());
  document.getElementById('delivery-submit-btn').addEventListener('click', () => {
    const carrier = document.getElementById('delivery-carrier').value || 'Internal Logistics';
    const tracking = document.getElementById('delivery-tracking').value || '';

    const qtyInputs = modalOverlay.querySelectorAll('.delivery-qty-input');
    let totalDeliveredItems = 0;
    let totalOrderedItems = 0;
    let hasShortageWarn = false;

    qtyInputs.forEach(input => {
      const idx = Number(input.getAttribute('data-index'));
      const delQty = Number(input.value || 0);
      const ordQty = order.items[idx].quantity;

      totalDeliveredItems += delQty;
      totalOrderedItems += ordQty;

      // Check stock reduction
      const invProduct = getInventoryProduct(order.items[idx].productId);
      if (invProduct) {
        if (delQty > invProduct.stock) {
          hasShortageWarn = true;
        }
        invProduct.stock = Math.max(0, invProduct.stock - delQty);
      }
    });

    if (totalDeliveredItems <= 0) {
      alert('Must deliver at least one item.');
      return;
    }

    if (hasShortageWarn) {
      alert('⚠️ stock deduction applied under negative inventory levels.');
    }

    if (!appState.salesDeliveries) {
      appState.salesDeliveries = [];
    }

    const nextDelId = appState.salesDeliveries.length + 1;
    const deliveryId = `DO-2026-${String(10000 + nextDelId).slice(1)}`;

    appState.salesDeliveries.push({
      id: deliveryId,
      orderId: order.id,
      ...
      status: totalDeliveredItems >= totalOrderedItems ? 'delivered' : 'shipped'
    });

    // Update statuses
    order.deliveryStatus = totalDeliveredItems >= totalOrderedItems ? 'delivered' : 'partially';
    order.status = 'processing';
    order.statusHistory.push({
      status: 'processing',
      date: todayIso(),
      notes: `Shipment ${deliveryId} dispatched via ${carrier}. Delivery Status: ${order.deliveryStatus}`
    });
      ...
      <tr class="hover:bg-slate-50 transition-colors border-b border-slate-100">
        ...
        <td class="p-4">
          <span class="px-2 py-0.5 rounded-full text-[9px] font-bold capitalize ${
            ...
            item.deliveryStatus === 'partially' ? 'bg-blue-50 text-blue-600' : 'bg-amber-50 text-amber-700'
          }">${escapeHtml(item.deliveryStatus)}</span>
        </td>
        <td class="p-4">
          <span class="px-2 py-0.5 rounded-full text-[9px] font-bold capitalize ${
