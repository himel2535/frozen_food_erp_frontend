...
export function InventoryStockMovement({
  ...
  const tiles = [
    { key: 'in', label: t('reports.inventory_stock_in'), value: movement.stockIn, icon: 'flat-color-icons:up', valueClass: 'text-emerald-600' },
    { key: 'out', label: t('reports.inventory_stock_out'), value: movement.stockOut, icon: 'flat-color-icons:down', valueClass: 'text-rose-600' },
    { key: 'adj', label: t('reports.inventory_adjustments'), value: movement.adjustments, icon: 'fluent-color:settings-24', valueClass: 'text-amber-600' },
    { key: 'net', label: t('reports.inventory_net_change'), value: movement.netChange, icon: 'fluent-color:data-trending-24', valueClass: 'text-blue-700' },
  ];
  ...
  return (
    <div className={`${IR_CARD} flex flex-col`}>
      ...
      <div className="grid grid-cols-2 gap-2 flex-1">
        {tiles.map((tile) => (
          <div key={tile.key} className="rounded-xl border border-slate-100 bg-slate-50/60 p-3 flex flex-col gap-1.5">
            ...
            <span className={`text-sm font-extrabold tabular-nums ${tile.valueClass}`}>{formatCurrency(tile.value)}</span>
          </div>
        ))}
      </div>
    </div>
