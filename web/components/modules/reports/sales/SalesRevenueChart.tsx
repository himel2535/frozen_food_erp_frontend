...
import type { SalesChartPoint } from '@/components/modules/reports/sales/sales-report-utils';
...
export function SalesRevenueChart({ data, onPrint }: SalesRevenueChartProps) {
  ...
  useEffect(() => {
    function draw(animate: boolean) {
      ...
      const max = Math.max(
        1,
        ...data.flatMap((point) => [point.thisMonth, point.lastMonth]),
      );
      ...
      const thisPts = pointsFor('thisMonth');
      const lastPts = pointsFor('lastMonth');
      const thisLine = pathFrom(thisPts);
      const lastLine = pathFrom(lastPts);
  ...
  return (
    <div className={`${SR_CARD} lg:col-span-2 flex flex-col`}>
      <ReportSectionHeader
        icon={<Icon icon="fluent-color:data-trending-24" width={26} height={26} className="shrink-0" />}
        title={t('reports.sales_revenue_overview')}
        onPrint={onPrint}
        ...
        action={
          <select className="bg-slate-50 border border-slate-200 text-[10px] font-semibold text-slate-600 rounded-lg px-2.5 py-1 cursor-pointer shrink-0">
            <option>{t('reports.sales_period_daily')}</option>
            <option>{t('reports.sales_period_weekly')}</option>
          </select>
        }
      />
      ...
      <div ref={boxRef} className="h-56 w-full relative">
        <svg className="w-full h-full" preserveAspectRatio="none">
          <defs>
            <linearGradient id="sales-revenue-area" x1="0" y1="0" x2="0" y2="1">
              <stop offset="0%" stopColor="#2563eb" stopOpacity="0.15" />
              <stop offset="100%" stopColor="#2563eb" stopOpacity="0" />
            </linearGradient>
          </defs>
          <path ref={thisAreaRef} d="" fill="url(#sales-revenue-area)" />
