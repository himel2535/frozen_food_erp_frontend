...
const NOTIFICATIONS: { icon: string; titleKey: string; meta: string }[] = [
  ...
  {
    icon: 'fluent-color:alert-badge-24',
    ...
    <div className="premium-card p-4 premium-shadow flex flex-col justify-between">
      <div className="flex items-center justify-between mb-4 border-b border-slate-100 pb-2">
        <div className="flex items-center gap-2 my-auto">
          ...
          <h3 className="text-sm font-bold text-slate-900 tracking-tight my-auto">{t('dashboard.notifications')}</h3>
        </div>
        <Link href="/notifications" className="text-[10px] font-bold text-blue-600 hover:text-blue-700 cursor-pointer my-auto">
      ...
      <div className="flex-1 flex flex-col gap-3.5 overflow-y-auto">
        {NOTIFICATIONS.map((item) => (
          <div key={item.titleKey} className="flex items-center gap-3">
            <Icon icon={item.icon} width={26} height={26} className="shrink-0 my-auto" />
